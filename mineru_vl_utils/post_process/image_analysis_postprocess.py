...
from .chart_invalid_repetition import try_fix_chart_invalid_repetition

from .mermaid_repetition import detect_mermaid_repetition_issues
from .mermaid_invalid_subgraph_lines import try_fix_mermaid_invalid_subgraph_lines
from .mermaid_node_id_legalize import try_fix_mermaid_node_id_legalization
from .mermaid_invalid_edge_lines import try_fix_mermaid_invalid_edge_lines
...
def extract_and_validate_mermaid_strict(content: str) -> str:
    ...
    # 步骤 3: 修复不规范的连线箭头
    # ==========================================
    # 先保护双向箭头，避免后续单向箭头修复误伤 <-->。
    bidir_token = "__MERMAID_BIDIR_ARROW__"
    code = re.sub(r"<\s*-\s*-\s*>", bidir_token, code)
    ...
    def node_fixer(match):
        ...
        # subgraph 声明行保持原样，避免误改成 subgraph_Downstream
        if raw_node_id.lower().startswith("subgraph "):
...
def process_image_or_chart(content: str) -> dict[str, str]:
    ...
    # 2) flowchart 类别：严格校验并提取 mermaid
        ...
        try:
            # 如果有重复置空
            if len(detect_mermaid_repetition_issues(normalized_content)) > 0:
                normalized_content = ""
            else:
                normalized_content = try_fix_mermaid_node_id_legalization(normalized_content)
                normalized_content = try_fix_mermaid_invalid_edge_lines(normalized_content)
                normalized_content = try_fix_mermaid_invalid_subgraph_lines(normalized_content)
        except BaseException as e:
            logger.warning("Failed to process flowchart: {}; content: {}", e, normalized_content)
    ...
    # 3) markdown 表格语法有误或不闭合：content 置空
        ...
        try:
            normalized_content = try_fix_chart_invalid_repetition(normalized_content)
        except Exception as e:
            logger.warning("Failed to process chart: {}; content: {}", e, normalized_content)
